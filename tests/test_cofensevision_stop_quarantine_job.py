# File: test_cofensevision_stop_quarantine_job.py
#
...
class TestStopQuarantineJobAction(unittest.TestCase):
    """Class to test the stop quarantine job action."""

    def setUp(self):
        """Set up method for the tests."""
        self.connector = CofenseVisionConnector()
        self.test_json = dict(config.TEST_JSON)
        self.test_json.update({"action": "stop quarantine job", "identifier": "stop_quarantine_job"})

        return super().setUp()

    @patch("cofensevision_utils.requests.put")
    def test_stop_quarantine_job_pass(self, mock_put):
        """Test the valid case for the stop quarantine job action.
        ...
        self.assertEqual(ret_val["result_data"][0]["message"], consts.VISION_SUCCESS_STOP_JOB_ACTION)

        mock_put.assert_called_with(
            f'{self.test_json["config"]["base_url"]}{consts.VISION_ENDPOINT_STOP_QUARANTINE_JOBS.format(job_id=job_id)}',
            headers=config.ACTION_HEADER,
            timeout=consts.VISION_REQUEST_TIMEOUT,
            verify=False)

    def test_stop_quarantine_job_invalid_id_fail(self):
        """Test the stop quarantine job action with invalid 'id' parameter."""
        self.test_json['parameters'] = [{
    ...
    @patch("cofensevision_utils.requests.put")
    def test_stop_quarantine_job_non_existing_fail(self, mock_put):
        """Test the stop quarantine job action with valid id, but the job does not exist.
