# File: cofensevision_connector.py
#
...
from cofensevision_utils import CofenseVisionUtils


class CofenseVisionConnector(BaseConnector):
    """Cofense Vision Connector class to interact with service API."""

    def __init__(self):
        """Prepare constructor for Cofense Vision."""
    ...
    def initialize(self):
        ...
        # If asset configuration is changed, regenerate the token for new user
        if self.state.get("client_id", "") != self.config["client_id"]:
            self.state.pop(consts.VISION_STATE_TOKEN, None)

        # Create the util object and use it throughout the action lifecycle
        self.util = CofenseVisionUtils(self)
    ...
    def handle_action(self, param):
        ...
        action_name = f"actions.cofensevision_{action_id}"
...
def main():  # pragma: no cover
    ...
    if username and password:
        try:
            ...
            r = requests.get(login_url, verify=verify, timeout=consts.VISION_REQUEST_TIMEOUT)
            ...
            r2 = requests.post(login_url, verify=verify, data=data, headers=headers, timeout=consts.VISION_REQUEST_TIMEOUT)
